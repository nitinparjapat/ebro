...
export default function OwnerDashboard() {
  ...
  const handleMediaSelect = async (field, files) => {
    ...
    try {
      const mediaItems = await Promise.all(
        selectedFiles.map(async (file) => ({
          id: `${field}-${file.name}-${file.lastModified}`,
          name: file.name,
          type: file.type,
      ...
      setProductForm((currentForm) => ({
        ...currentForm,
        [field]: [...currentForm[field], ...mediaItems],
      }));
      setDashboardError("");
      setDashboardMessage("");
    } catch {
  ...
  const removeMedia = (field, mediaIndex) => {
    setProductForm((currentForm) => ({
      ...currentForm,
      [field]: currentForm[field].filter((_, index) => index !== mediaIndex),
    }));
  };
  ...
  const handleConfirmOrder = async (order) => {
    ...
    setDashboardError("");
    setDashboardMessage(`${order.code} confirmed.`);
  };
              ...
              <div>
                ...
                <label className="mt-2 flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-gray-300 bg-gray-50 px-3 py-4 text-sm font-semibold text-gray-700">
                  ...
                  <input
                    ...
                    multiple
                    onChange={(event) =>
                      handleMediaSelect("images", event.target.files)
                    }
                    className="sr-only"
                  />
                </label>
              ...
              <div>
                ...
                <label className="mt-2 flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-gray-300 bg-gray-50 px-3 py-4 text-sm font-semibold text-gray-700">
                  ...
                  <input
                    ...
                    multiple
                    onChange={(event) =>
                      handleMediaSelect("videos", event.target.files)
                    }
                    className="sr-only"
                  />
                </label>
